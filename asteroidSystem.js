class AsteroidSystem {
  ...
  constructor(){
    ...
    this.accelerations = [];
    this.diams = [];
    //free to use image located
    //https://www.dlf.pt/ddetail/TRbTJbo_illustration-of-mercury-mercury-planet-with-face-hd/
    //author: DLF.pt
    //Access Date: Dec 15 2021
    this.astroid = loadImage('./assets/happyAstroid.png');
  }
  ...
  draw(){
    ...
    for (var i=0; i<this.locations.length; i++){
      ...
      //draws the image of the astroid that is falling
      imageMode(CENTER);
      image(this.astroid,this.locations[i].x, this.locations[i].y,this.diams[i], this.diams[i]);
    }
  }
